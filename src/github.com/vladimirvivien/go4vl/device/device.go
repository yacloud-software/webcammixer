...
func Open(path string, options ...Option) (*Device, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("device open (2): %s: %w", path, err)
	}
	dev.cap = cap
	...
	if !dev.cap.IsStreamingSupported() {
		return nil, fmt.Errorf("device open (3): device does not support streamingIO")
	}
	...
	default:
		/*
			if err := v4l2.CloseDevice(dev.fd); err != nil {
				return nil, fmt.Errorf("device open (4): %s: closing after failure: %s", path, err)
			}
			return nil, fmt.Errorf("device open (5): %s: %w", path, v4l2.ErrorUnsupportedFeature)
		*/
	}
	...
	if !reflect.ValueOf(dev.config.pixFormat).IsZero() {
		if err := dev.SetPixFormat(dev.config.pixFormat); err != nil {
			return nil, fmt.Errorf("device open (6): %s: set format: %w", path, err)
		}
	} else {
		dev.config.pixFormat, err = v4l2.GetPixFormat(dev.fd, v4l2.BufTypeVideoCapture)
		/*
			//  ignore failure, we can and should do that later
					if err != nil {
						return nil, fmt.Errorf("device open (7): %s: get default format: %w", path, err)
					}
		*/
	}
	...
	if !reflect.ValueOf(dev.config.fps).IsZero() {
		if err := dev.SetFrameRate(dev.config.fps); err != nil {
			return nil, fmt.Errorf("device open (8): %s: set fps: %w", path, err)
		}
	} else {
		dev.config.fps, err = dev.GetFrameRate()
		/*
			//  ignore failure, we can and should do that later
					if err != nil {
						return nil, fmt.Errorf("device open (9): %s: get fps: %w", path, err)
					}
		*/
	}
...
func (d *Device) GetFormatDescriptions() ([]v4l2.FormatDescription, error) {
	...
	*/
	return v4l2.GetAllFormatDescriptions(d.fd, v4l2.BufTypeVideoCapture)
}
...
func (d *Device) startStreamLoop(ctx context.Context) error {
	...
	go func() {
		...
		for {
			...
			case <-waitForRead:
				if d.stop_stream {
					// catch a race condition:Stop() cleans up buffers but we're still reading.
					return
				}
...
// low-level function to get the underlying filehandle
func (d *Device) GetFile() *os.File {
	return d.file
}

// low-level function go get the OS FD Descriptor
func (d *Device) GetFD() uintptr {
	return d.fd
}
